...
function MobilesTablets() {
    ...
    const latestProduct = async () => {
        try {
            ...
            setLoading(true);
            setErrors({});
            const {data} = await api.getTwoSlug('mobile-tablet', 'latest_product');
    ...
    const bestSellingProduts = async () => {
        try {
            ...
            setLoading(true);
            setErrors({});
            const {data} = await api.getTwoSlug('mobile-tablet', 'best_selling');
    ...
    const topRatingProduts = async () => {
        try {
            ...
            setLoading(true);
            setErrors({});
            const {data} = await api.getTwoSlug('mobile-tablet', 'top_rating');
    ...
    const featureProduts = async () => {
        try {
            ...
            setLoading(true);
            setErrors({});
            const {data} = await api.getTwoSlug('mobile-tablet', 'featured');
            ...
            <section className="section-maker">
                <div className="container">
                    <div className="sec-maker-header text-center">
                        <h3 className="sec-maker-h3">Mobils &amp; Tablets</h3>
